  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [new File([blob], "card.png", { type: "image/png" })] })) {
        const file = new File([blob], `红薯医生诊断-${title.slice(0, 10)}.png`, { type: "image/png" });
        await navigator.share({ files: [file], title: "红薯医生诊断卡片" });
        return;
      }
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.download = `红薯医生诊断-${title.slice(0, 10)}.png`;
      link.href = url;
      link.click();
      ...
      */}
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ fontSize: 11, fontWeight: 600, opacity: 0.8, marginBottom: 8 }}>红薯医生诊断</div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: "#262626" }}>红薯医生</span>
          </div>
